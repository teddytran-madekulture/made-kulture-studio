-- Seed / sync the 10 orientation modules (v1) from the finalized curriculum.
-- Upserts by (slug, version) so re-running syncs content (body/quiz/etc).
-- retake_on_miss = true on house-rules, access, and safety.
-- Worker-facing copy refers to "management" (general contact) and "the owner"
-- (the across-the-street / owner-managed references) — never a personal name.
...
You are the studio's presence when the owner isn't on-site: keep it running smoothly, clean, on-time, and by the rules.

**Reaching management:** call management directly. The owner lives across the street — if you can't reach anyone by phone in an emergency, go get them.$body$,
...
{"id":"q2","prompt":"How do you reach management with an urgent question?","type":"single","options":["Email only","Call management directly; if unreachable in an emergency, go to the owner's place across the street","Wait for someone to check in","Post in a group chat"],"answer":[1]},
{"id":"q3","prompt":"You may let a client into their set before their booked start time.","type":"boolean","options":["True","False"],"answer":[1]}
]}$quiz$::jsonb, 1, true)
on conflict (slug, version) do update set title = excluded.title, body = excluded.body, required_for = excluded.required_for, quiz = excluded.quiz, sort_order = excluded.sort_order, active = excluded.active, updated_at = now();
...
Sets A, B, C, D, Concrete, Vintage, Cottage are $40/hr. **Set C** has the red vinyl seamless. **The Watering Hole** ($75/hr, 2-hr min) is the shallow black pool — **owner-managed:** the owner handles it and it is **not offered when the owner is off-site.** **Studio One** ($65/hr) is reached by a steep ramp.$body$,
...
{"id":"q2","prompt":"Who handles The Watering Hole, and when is it available?","type":"single","options":["Any worker, anytime","The owner handles it; not offered when the owner is off-site","Front desk only","Interns"],"answer":[1]},
...
{"id":"q3","prompt":"You see a messy setup and can't tell if it was approved. What do you do?","type":"single","options":["Allow it","Refuse outright","Text/call management to confirm before allowing it","Charge extra and allow it"],"answer":[2]}
]}$quiz$::jsonb, 3, true)
on conflict (slug, version) do update set title = excluded.title, body = excluded.body, required_for = excluded.required_for, quiz = excluded.quiz, sort_order = excluded.sort_order, active = excluded.active, updated_at = now();

insert into onboarding_modules (slug, title, body, version, required_for, quiz, sort_order, active) values
('access', 'Access, Security & Closing Up',
$body$Both doors are on igloohome smart locks. Each booking gets **two separate per-booking codes — one for the front door and one for the back** (labeled Front/Back in the confirmation); a single code can't open both. A set **unlocks at the booked start time — no early entry.** Your shift code auto-expires; don't share it. **There is no separate alarm** — security is the locks.
...
{"id":"q2","prompt":"Each booking gets a separate code for the front door and the back door (one code can't open both).","type":"boolean","options":["True","False"],"answer":[0]},
...
**Closeout photo:** after your final touch-up, take a wide "after" photo of each used set matched to its reference look — it's your proof of work and the studio's evidence in a cleaning-fee dispute. Low on supplies? **Text management.** The $150 cleaning fee is what a clean handoff prevents.$body$,
1, '{attendant,sanitation,intern,freelancer}'::worker_class[],
$quiz${"pass_pct":80,"retake_on_miss":false,"questions":[
...
{"id":"q3","prompt":"You're low on paper towels. What do you do?","type":"single","options":["Ignore it","Text management","Buy them yourself","Close the restroom"],"answer":[1]}
]}$quiz$::jsonb, 6, true)
on conflict (slug, version) do update set title = excluded.title, body = excluded.body, required_for = excluded.required_for, quiz = excluded.quiz, sort_order = excluded.sort_order, active = excluded.active, updated_at = now();
...
{"id":"q2","prompt":"A booking is running over. What do you do?","type":"single","options":["Silently charge an extra hour","Give a heads-up and handle the overage per management's direction","Lock them out","Ignore it"],"answer":[1]},
...
$body$**Emergencies:** anything life-threatening or a fire → **call 911 first, then call management.** Everything else → call management (reply within a few minutes; if unreachable, go get the owner across the street). Report incidents with what/when/who + a photo if there's damage.
...
1, '{attendant,sanitation,intern,freelancer}'::worker_class[],
$quiz${"pass_pct":80,"retake_on_miss":true,"questions":[
{"id":"q1","prompt":"What's the order for a fire or serious injury?","type":"single","options":["Call management first","Call 911 first, then call management","Wait and see","Post online"],"answer":[1]},
{"id":"q2","prompt":"Who operates the gas heat (Nest)?","type":"single","options":["Any worker","The owner — owner-managed; don't touch unless directed","Front desk","Clients"],"answer":[1]},
...
1, '{attendant,sanitation,intern,freelancer}'::worker_class[],
$quiz${"pass_pct":80,"retake_on_miss":false,"questions":[
{"id":"q1","prompt":"How do you start a shift?","type":"single","options":["Just show up","Clock in at the kiosk/app at your start time","Text management","Sign a paper sheet"],"answer":[1]},
