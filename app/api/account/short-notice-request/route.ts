...
import { shortNoticeActive, shortNoticeViewActive } from '@/lib/short-notice'
...
export async function GET() {
  ...
  return NextResponse.json({ canView, canBook, latest })
}
...
export async function POST(req: NextRequest) {
  ...
  const row = {
    ...
    status:         'pending',
    desired_date:   (typeof body.desiredDate === 'string' && body.desiredDate) ? body.desiredDate : null,
    desired_start:  (body.desiredStart != null && !isNaN(Number(body.desiredStart))) ? Number(body.desiredStart) : null,
  ...
  await Promise.allSettled([
    sendShortNoticeRequestAlert({ customerName: c.name, customerEmail: c.email, desiredDate: row.desired_date, desiredStart: row.desired_start, note: row.note, approveUrl }),
    sendOwnerSMS(`🔔 Short-notice request from ${c.name}${row.desired_date ? ` for ${row.desired_date}` : ''}. Approve: ${approveUrl}`),
  ])
