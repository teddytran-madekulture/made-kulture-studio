...
export async function POST(req: NextRequest) {
  ...
  const { data: booking, error } = await supabase
    ...
    // manage_token is generated by the column DEFAULT (migration 101), so it
    // only exists on the returned row — select it or the confirmation email
    // below silently falls back to /account, which a GUEST cannot open.
