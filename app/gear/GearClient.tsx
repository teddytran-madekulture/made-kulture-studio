...
export default function GearClient({ content = {} }: { content?: PageContent }) {
  ...
  return (
    <div style={{ background: '#080808', minHeight: '100vh', color: '#fff', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ padding: isMobile ? '110px 20px 28px' : '128px 40px 40px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ fontFamily: 'Anton, "Bebas Neue", sans-serif', fontSize: isMobile ? 44 : 64, letterSpacing: '0.02em', lineHeight: 1 }}>{c.headline}</div>
        <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', maxWidth: 640, marginTop: 16, lineHeight: 1.6 }}>
          {nl(c.intro)}
        </p>
        <Link href={c.ctaHref || '/book'} style={{ display: 'inline-block', marginTop: 20, background: '#fff', color: '#080808', padding: '12px 24px', textDecoration: 'none', fontFamily: 'Inter', fontSize: 11, fontWeight: 600, letterSpacing: '0.12em' }}>
          {c.ctaLabel}
        </Link>
      </div>
