...
export default function AdminRolesPage() {
  ...
  const loadRoles = () => {
    fetch('/api/admin/roles')
      .then(async r => {
        ...
        setRoles(d.roles ?? []); setLoading(false)
      })
  ...
  return (
    <div style={wrap}><div style={inner}>
      <h1 style={{ fontFamily: 'Anton, "Bebas Neue", sans-serif', fontSize: 34, letterSpacing: '0.02em', margin: '0 0 6px' }}>DIRECTORY ROLES</h1>
      <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: '0 0 28px' }}>
        The creative roles shown across signup, profiles, and the directory. Add your own, or remove any you don&apos;t want — including the built-in ones.
      </p>

      <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: 20 }}>
        ...
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase', margin: '22px 0 10px' }}>
          Active roles{loading ? '' : ` (${active.length})`}
        </div>
        {loading ? (
