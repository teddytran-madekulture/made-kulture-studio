    ...
    <main style={{ background: C.bg, minHeight: '100vh', color: C.text, padding: '40px 24px 80px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 8 }}>
          <h1 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 40, letterSpacing: '0.03em', margin: 0 }}>EDIT WEBSITE</h1>
        ...
        {CONTENT_PAGES.length > 1 && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
            {CONTENT_PAGES.map(p => (
              <button key={p.slug} onClick={() => setPageSlug(p.slug)} style={{ background: p.slug === pageSlug ? '#fff' : 'transparent', color: p.slug === pageSlug ? '#080808' : C.text, border: `1px solid ${C.line}`, padding: '8px 14px', borderRadius: 6, fontSize: 13, cursor: 'pointer' }}>{p.label}</button>
            ))}
          </div>
        )}
