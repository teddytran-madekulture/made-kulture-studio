...
function BookingWizard({ content = {} }: { content?: PageContent }) {
  ...
  const tok = (t: string) => (t ?? '')
    .replace(/\{min\}/g, String(minSetRate))
    .replace(/\{max\}/g, String(maxSetRate))
    .replace(/\{rate\}/g, ratesLoaded ? String(buyoutRate) : '\u2026')
        ...
        {step === 1 && booking.type === null && (
          <StepWrapper title="HOW WOULD YOU LIKE TO BOOK?">
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 1, background: 'rgba(255,255,255,0.06)' }}>
              {[
                { type: 'set' as BookingType,    label: c.setCardTitle,    sub: tok(c.setCardSub),    limit: c.setCardLimit,    price: `FROM $${minSetRate}/HR` },
                { type: 'studio' as BookingType, label: c.studioCardTitle, sub: tok(c.studioCardSub), limit: c.studioCardLimit, price: ratesLoaded ? `$${buyoutRate}/HR · ${STUDIO_MIN_HOURS}HR MIN` : 'LOADING RATE…' },
              ].map(opt => (
            ...
            {booking.type === 'set' && (
              <div style={{ marginBottom: 24 }}>
                ...
                {(currentComplete || setCart.length > 0) && (
                  // Deliberately NOT styled as a second primary button. CONTINUE
                  // stays the white one — most bookings are a single set, and two
...
export default function BookClient({ content = {} }: { content?: PageContent }) {
  return (
    <Suspense fallback={<div style={{ background: '#080808', minHeight: '100vh' }} />}>
      <BookingWizard content={content} />
    </Suspense>
  )
}
